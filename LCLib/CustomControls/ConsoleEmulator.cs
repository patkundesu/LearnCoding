...
namespace LCLib
{
    ...
    public class ConsoleEmulator : TextBox
    {
        /// <summary>
        ...
        public ConsoleEmulator()
        {
            ...
            KeyDown += ConsoleEmulator_KeyDown;
            PreviewKeyDown += ConsoleEmulator_PreviewKeyDown;
        }
        ...
        private void ConsoleEmulator_KeyDown(object sender, KeyEventArgs e)
        {
            bool read_only = (SelectionStart < inputStart);
            if (!procStart || read_only)
                e.Handled = true;
            if (e.Key == Key.Return)
            {
                string input = Text.Substring(inputStart, Text.Length - inputStart);
                Text += "\n";
                WriteInput(input);
            }
        }
        ...
        private void FireProcessInput(ProcessEventArgs args)
        {
            var theEvent = OnInput;
            if (theEvent != null)
                theEvent(this, args);
        }
        private void FireProcessOutput(ProcessEventArgs args)
        {
            var theEvent = OnOutput;
            if (theEvent != null)
                theEvent(this, args);
        }
        private void FireProcessError(ProcessEventArgs args)
        {
            var theEvent = OnError;
            if (theEvent != null)
                theEvent(this, args);
        }
        private void FireProcessExit(ProcessEventArgs args)
        {
            var theEvent = OnExit;
            if (theEvent != null)
                theEvent(this, args);
        ...
        private void ConsoleEmulator_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            bool read_only = (SelectionStart < inputStart);
    ...
    /// <summary>
    /// Process Event Arguments for Console Emulator
    /// </summary>
